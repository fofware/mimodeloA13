...
export class eMailValidation {
  ...
  static exist(service:AuthService) {
    return async (AC: AbstractControl): Promise<Observable<ValidationErrors> | null>  => {
      if(AC.value){
        const ret = await service.mailExist(AC.value);
        console.log(ret);
  ...
  static other(service:AuthService) {
...
export function emailAllreadyExist(
  control: AbstractControl, authService: AuthService
): { [key: string]: boolean } | null {
  let ret = null;
  const value = authService.emailFind(control.value).subscribe((data:any) =>{
    console.log(value)
  })
  return ret;
}
