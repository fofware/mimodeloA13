...
class TallaControlers {
  ...
  async list(req: Request, res: Response){
    ...
    const count = await tallas.count(filter);
    ...
    let ret = {}
    try {
      const rows = await tallas.find(filter).limit(params.limit).skip(params.offset).sort(params.sort);
      status = 200;
      ret = {
  ...
  async add(req: Request, res: Response){
    ...
    try {
      let ret = await tallas.findOneAndUpdate(filter, update, {
        new: true,
        upsert: true,
        rawResult: true // Return the raw result from the MongoDB driver
      });

      ret.value instanceof tallas; // true
  ...
  async delete(req: Request, res: Response){
    ...
    let status = 0;
    try {
      ret['data'] = await tallas.findByIdAndDelete(params._id);
  ...
  async put(req: Request, res: Response){
    ...
    let ret = {};
    try {
      ret['data'] = await tallas.findByIdAndUpdate(params._id, params);
...
export const tallaCtrl = new TallaControlers();