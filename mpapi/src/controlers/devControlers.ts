...
export class devControler {
  ...
  config(){
    this.router.get('/mediosdepagos', this.mediosdepagos );
    this.router.get('/tipodedocumentos', this.tipodedocumentos );
    this.router.get('/cajas', this.cajas );
    this.router.get('/caja/:id', this.caja );
    this.router.get('/intencionesdepago', this.intencionesdepago );
    this.router.get('/pagos', this.pagos );
    this.router.get('/devices', this.devices );
    this.router.get('/ordenes', this.ordenes );
  }
  /*
    curl -X GET \
  ...
  */
  async ordenes(req: Request, res: Response){
    ...
    const options = {
      host: 'api.mercadopago.com',
      path: `/merchant_orders/search${qry}`,
      method: 'GET',
      port: 443,
      headers: {
        'Accept': 'application/json',
