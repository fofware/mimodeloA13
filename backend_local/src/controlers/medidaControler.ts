...
class MedidaControlers {
  ...
  config () {
    this.router.get('/medidas',
				//passport.authenticate('jwt', {session:false}), 
				this.list );
    this.router.get('/medidas/tah',
				//passport.authenticate('jwt', {session:false}), 
				this.listtah );
  ...
  async list(req: Request, res: Response){
    ...
    const count = await medidas.count(filter);
    ...
    let ret = {}
    try {
      const rows = await medidas.find(filter).limit(params.limit).skip(params.offset).sort(params.sort);
      status = 200;
      ret = {
  ...
  async get(req: Request, res: Response){
    ...
    const ret = await medidas.findById( params.id );
  ...
  async add(req: Request, res: Response){
    ...
    try {
      let ret = await medidas.findOneAndUpdate(filter, update, {
        new: true,
        upsert: true,
        rawResult: true // Return the raw result from the MongoDB driver
      });

      ret.value instanceof medidas; // true
  ...
  async delete(req: Request, res: Response){
    ...
    let status = 0;
    try {
      ret['data'] = await medidas.findByIdAndDelete(params._id);
  ...
  async put(req: Request, res: Response){
    ...
    let ret = {};
    try {
      ret['data'] = await medidas.findByIdAndUpdate(params._id, params);
...
export const MedidaCtrl = new MedidaControlers();